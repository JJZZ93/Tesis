 ...
 * @author josezamora
 */
public class singleHuman2 extends Agent{
    ...
    private class SuscribirCatalogo extends SubscriptionInitiator {
        ...
        protected void handleAgree(ACLMessage inform) {
            System.out.println(singleHuman2.this.getLocalName() + ": Solicitud aceptada.");
        }
 
        // Maneja la respuesta en caso que rechace: REFUSE
 
        protected void handleRefuse(ACLMessage inform) {
            System.out.println(singleHuman2.this.getLocalName() + ": Solicitud rechazada.");
        }
 
        //Maneja la informacion enviada: INFORM
 
        protected void handleInform(ACLMessage inform) {
            System.out.printf("%s: Informe recibido: %s.\n", singleHuman2.this.getLocalName(), inform.getContent());
        ...
        protected void handleFailure(ACLMessage failure) {
            ...
            if (failure.getSender().equals(myAgent.getAMS())) {
                System.out.println(singleHuman2.this.getLocalName() + ": El destinatario no existe.");
            } else {
                System.out.printf("%s: El agente %s falló al intentar realizar la acción solicitada.\n",
                    singleHuman2.this.getLocalName(), failure.getSender().getName());
            }
        }
        ...
        public void cancellationCompleted(AID agente) {
            ...
            if (msg.getPerformative() == ACLMessage.INFORM)
                System.out.printf("%s : Suscripcion cancelada con el agente %s.\n",
                    singleHuman2.this.getLocalName(), agente.getLocalName());
            else
